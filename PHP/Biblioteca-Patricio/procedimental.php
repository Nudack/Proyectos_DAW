...
// Creando la conexión utilizando mysqli
...
switch($oper) {
    ...
    case 'delete':
        eliminar();
        header("location: /biblioteca_mysqli.php");
        exit(0);
        break;
...
function cabecera($titulo_seccion = '') {
    $breadcrumb = empty($titulo_seccion) ? "<li class=\"breadcrumb-item\">biblioteca</li>" :
        "<li class=\"breadcrumb-item\"><a href=\"/biblioteca_mysqli.php\">biblioteca</a></li>
...
function formulario($oper, $errores = []) {
    ...
    if ($_POST['paso'] && count($errores) == 0) {
        ...
        $disabled = 'disabled';
        $botones_extra = '<a href="/biblioteca_mysqli.php?oper=create" class="btn btn-primary">Nuevo Libro</a>';
        
        if ($oper == 'update') {
            $botones_extra .= ' <a href="/biblioteca_mysqli.php?oper=update&id=' . $id . '" class="btn btn-primary">Editar</a>';
        }
    }
    ...
    return "
        <form method=\"POST\" action=\"biblioteca_mysqli.php\">
...
function insertar() {
    ...
    $sql = "
        INSERT INTO libros (nombre, descripcion, autor, editorial, ip_alta)
        VALUES ('{$_POST['nombre']}', '{$_POST['descripcion']}', '{$_POST['autor']}', '{$_POST['editorial']}', '{$_SERVER['REMOTE_ADDR']}')
    ";
    $resultado = mysqli_query($conexion, $sql);
}
...
function resultados_busqueda() {
    ...
    if ($resultado && mysqli_num_rows($resultado) > 0) {
        while ($fila = mysqli_fetch_assoc($resultado)) {
            $listado_libros .= "
                <tr>
                    <th scope=\"row\">
                        <a href=\"/biblioteca_mysqli.php?oper=update&id={$fila['id']}\" class=\"btn btn-primary\">Actualizar</a>
                        <a onclick=\"if(confirm('Cuidado, estás tratando de eliminar el libro: {$fila['nombre']}')) location.href = '/biblioteca_mysqli.php?oper=delete&id={$fila['id']}';\" class=\"btn btn-danger\">Eliminar</a>
                    </th>
                    <td>{$fila['nombre']}</td>
    ...
    $pagina_anterior = $pagina ? '<li class="page-item"><a class="page-link" href="/biblioteca_mysqli.php?pagina='. ($pagina - 1) .'">Anterior</a></li>' : '';
    ...
    $listado_libros .= '
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                '. $pagina_anterior .'
                <li class="page-item"><a class="page-link" href="/biblioteca_mysqli.php?pagina='. ($pagina + 1) .'">Siguiente</a></li>
            </ul>
        </nav>

        <div class="alta">
            <a href="/biblioteca_mysqli.php?oper=create" class="btn btn-success">Alta de libro</a>
        </div>
    ';
