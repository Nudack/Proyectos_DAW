$(document).ready(function() {
    ...
    class Avion {
        constructor(nombre, filas, columnas, precios, categoria) {
            ...
            this.categoria = categoria; // Dependiendo de la posicion del asiento asignar la categoria, averiguar como hacerlo, seguramente con un array y quitar el codigo html 
        }
    ...
    function mostrarAsientos() {
        ...
        $('#tablaAsientos').appendChild(tabla);
    ...
    function mostrarComplementos(categoria) {
        ...
        $('#complementos').innerHTML = complementosHtml;
    }

    // Función para actualizar el precio total según la selección de asientos y complementos
    function actualizarPrecio() {
        const categoria = $('#categoria').value; // Cambiar este valor por el del constructor
        let avion = obtenerAvionSeleccionado();
        ...
        if (categoria === 'lowCost') {
            if ($('#maleta10kg') && document.getElementById('maleta10kg').checked) total += 30;
            if ($('#maleta25kg') && $('#maleta25kg').checked) total += 45;
            if ($('#embarque') && $('#embarque').checked) total += 10;
            if ($('#menu') && $('#menu').checked) total += 20;
        } else if (categoria === 'economica') {
            if ($('#maleta25kg') && $('#maleta25kg').checked) total += 45;
            if ($('#menu') && $('#menu').checked) total += 20;
        } else if (categoria === 'business') {
        ...
        const residente = $('#residente').checked;
        if (residente) {
    ...
    function seleccionarAsiento(fila, columna) {
        ...
        if (avion.asientos[fila][columna]) {
            asientosSeleccionados.forEach(
                // Terminarlo para que no pueda añadir el mismo asiento todo el rato y aumentar el precio total
            )
